...
type SalesPostgresTestSuite struct {
	suite.Suite
...
func TestSalesPostgresTestSuit(t *testing.T) {
	t.Parallel()

	suite.Run(t, new(SalesPostgresTestSuite))
}

func (suite *SalesPostgresTestSuite) SetupTest() {
	var err error
	...
	err = suite.db.AutoMigrate(&postgres.SaleModel{}, &postgres.SaleItemModel{})
	assert.NoError(suite.T(), err)
}

func (suite *SalesPostgresTestSuite) TeardownTest() {
	_ = suite.container.Terminate(suite.ctx)
}

func (suite *SalesPostgresTestSuite) TestCRUD() {
