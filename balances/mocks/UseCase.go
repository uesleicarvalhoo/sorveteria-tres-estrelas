...
func (_m *UseCase) GetCashFlowBetween(ctx context.Context, startAt time.Time, endAt time.Time) (balances.CashFlow, error) {
	ret := _m.Called(ctx, startAt, endAt)

	var r0 balances.CashFlow
	if rf, ok := ret.Get(0).(func(context.Context, time.Time, time.Time) balances.CashFlow); ok {
